...
from app import ma # Import ma for schema usage
from app.schemas import RecurringTransactionSchema # Załóżmy, że ta schema istnieje
...
def add_recurring_transaction():
    try:
        ...
        rec_tx = create_recurring_transaction(current_user.id, data)
...
def list_recurring_transactions():
    transactions = get_all_recurring_transactions(current_user.id)
...
def edit_recurring_transaction(rec_tx_id):
    try:
        ...
        updated_tx = update_recurring_transaction(current_user.id, rec_tx_id, data)
...
def remove_recurring_transaction(rec_tx_id):
    try:
        delete_recurring_transaction(current_user.id, rec_tx_id)
