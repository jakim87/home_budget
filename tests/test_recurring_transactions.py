...
from app.services.recurring_service import (
    _calculate_next_run_date_for_recurring,
...
def setup_for_recurring(app, test_user_token):
    ...
    account = Account(name="Konto Cykliczne", bank_name="Bank", balance=Decimal("1000.00"), user_token=test_user_token)
...
def test_api_add_recurring_transaction(logged_in_client, app, setup_for_recurring):
    """Sprawdza, czy można dodać definicję transakcji cyklicznej przez API."""
    user_token, account_id, category_id = setup_for_recurring

    response = logged_in_client.post('/api/recurring-transactions/', json={
...
def test_api_list_recurring_transactions(logged_in_client, app, setup_for_recurring):
    """Sprawdza, czy API poprawnie listuje definicje transakcji cyklicznych."""
    user_token, account_id, category_id = setup_for_recurring
    ...
    response = logged_in_client.get('/api/recurring-transactions/')
...
def test_api_update_recurring_transaction(logged_in_client, app, setup_for_recurring):
    """Sprawdza, czy można zaktualizować definicję transakcji cyklicznej przez API."""
    user_token, account_id, category_id = setup_for_recurring
    ...
    response = logged_in_client.put(f'/api/recurring-transactions/{rec_tx_id}', json=updated_data)
...
def test_api_delete_recurring_transaction_blocked(logged_in_client, app, setup_for_recurring):
    """
    ...
    response = logged_in_client.delete(f'/api/recurring-transactions/{rec_tx_id}')
...
# --- Przypadki brzegowe wyliczania dat ---

def test_monthly_day31_clamps_to_month_end(app, setup_for_recurring):
    """day_of_month=31 w miesiącu krótszym musi trafić na ostatni dzień miesiąca (nie błąd)."""
    user_token, account_id, _ = setup_for_recurring
    rec = RecurringTransaction(
        user_token=user_token, account_id=account_id, title="Rata",
        amount=Decimal("-100.00"), frequency=Frequency.MONTHLY, day_of_month=31,
        interval=1, start_date=date(2024, 1, 31), next_run_date=date(2024, 1, 31),
    )
    next_date = _calculate_next_run_date_for_recurring(rec, date(2024, 1, 31))
    assert next_date == date(2024, 2, 29)  # 2024 = rok przestępny

def test_weekly_aligns_to_day_of_week(app, setup_for_recurring):
    """Cotygodniowa transakcja wyrównuje kolejny termin do zadanego dnia tygodnia."""
    user_token, account_id, _ = setup_for_recurring
    rec = RecurringTransaction(
        user_token=user_token, account_id=account_id, title="Basen",
        amount=Decimal("-30.00"), frequency=Frequency.WEEKLY, day_of_week=0,  # poniedziałek
        interval=1, start_date=date(2024, 6, 3), next_run_date=date(2024, 6, 3),
    )
    next_date = _calculate_next_run_date_for_recurring(rec, date(2024, 6, 3))  # pon 3.06
...
def test_process_deactivates_recurring_after_end_date(app, setup_for_recurring):
    ...
    user_token, account_id, _ = setup_for_recurring
    rec = RecurringTransaction(
        user_token=user_token, account_id=account_id, title="Zakończona subskrypcja",
        amount=Decimal("-20.00"), frequency=Frequency.MONTHLY, day_of_month=1,
        start_date=date(2024, 1, 1), next_run_date=date(2024, 6, 1),
