...
from app import db
from app.models import Category, Contractor, Account, Transaction
...
def init_data():
    ...
    contractors = db.session.query(Contractor).filter_by(user_id=current_user.id, is_active=True).all()
    ...
    accounts = db.session.query(Account).filter_by(user_id=current_user.id, is_active=True).all()
