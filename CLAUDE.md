...
**Strony informacyjne**: `legal_bp.py` serwuje `/regulamin`, `/polityka-prywatnosci`, `/o-aplikacji` i `/kalkulator-kredytu` — statyczne szablony dziedziczące po `legal_base.html`, BEZ `@login_required` (linkujemy je z modalu rejestracji). Nazwa administratora i adres kontaktowy pochodzą z `config.APP_ADMIN_NAME` / `APP_CONTACT_EMAIL` (nadpisywalne z `.env`), autor z `APP_AUTHOR` — nie wpisuj tych danych na sztywno w szablonach. Kalkulator kredytu (`kalkulator_kredytu.js`, bez prefiksu — nie jest modułem SPA) liczy wyłącznie w przeglądarce, więc jest publiczny bez ryzyka: raty równe/malejące, prowizja, opłaty, RRSO (bisekcja, z planu bez nadpłat), nadpłaty (skrócenie okresu / obniżenie raty) i test wzrostu stóp. Odsetki = saldo × stopa / 12 z zaokrągleniem do grosza — tak liczy Pekao; testy vitest odtwarzają raty z prawdziwego harmonogramu kredytu. Linki żyją w dwóch partialach: `_footer.html` (stopka SPA i stron informacyjnych) oraz `_auth_legal_links.html` (modal logowania/rejestracji).
...
Demo ma też niepustą poczekalnię (`DEMO_STAGING_TRANSFERS`): parę nóg przelewu i jedną nogę bez pary, żeby oba stany plakietki `transfer_pair` były widoczne bez importowania czegokolwiek. Towarzyszący im wpis `StatementImport` powstaje **po** `_generate_history` i **tylko dla konta osobistego** — jest dla aplikacji sygnałem „to konto dostaje własne wyciągi", więc wcześniej albo szerzej wyłączyłby lustra comiesięcznym przelewom z historii i salda by się rozjechały.
