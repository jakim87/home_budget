...
def add_planned_transaction():
    try:
        ...
        planned_tx = create_planned_transaction(current_user.id, data)
...
def list_planned_transactions():
    transactions = get_all_planned_transactions(current_user.id)
...
def remove_planned_transaction(pt_id):
    try:
        delete_planned_transaction(current_user.id, pt_id)
