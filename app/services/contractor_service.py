...
def create_contractor(user_id, data):
    try:
...
def update_contractor(user_id, c_id, data):
    try:
        cont = db.session.query(Contractor).filter_by(id=c_id, user_id=user_id).first()
        if not cont:
...
def soft_delete_contractor(user_id, c_id):
    try:
        cont = db.session.query(Contractor).filter_by(id=c_id, user_id=user_id).first()
        if cont:
