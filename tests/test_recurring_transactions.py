...
@pytest.fixture
def setup_for_recurring(app, test_user_id):
    """Przygotowuje konto i kategorię dla testów transakcji cyklicznych."""
    with app.app_context():
        account = Account(name="Konto Cykliczne", bank_name="Bank", balance=1000.0, user_id=test_user_id)
        category = Category(name="Czynsz", type="expense")
        db.session.add_all([account, category])
        db.session.commit()
        return test_user_id, account.id, category.id
...
def test_api_add_recurring_transaction(client, app, setup_for_recurring):
    ...
    """
    user_id, account_id, category_id = setup_for_recurring
    login_user_helper(client)

    # ACTION - symulujemy wysłanie nowej definicji
...
def test_api_list_recurring_transactions(client, app, setup_for_recurring):
    ...
    # SETUP - dodajemy dwie definicje transakcji cyklicznych
    with app.app_context():
        ...
        # Dodajemy transakcję dla innego użytkownika, która nie powinna być widoczna
        other_user = User(username="otheruser", email="other@test.com", password_hash=generate_password_hash("password"))
        db.session.add(other_user)
        db.session.commit()
        rt3 = RecurringTransaction(user_id=other_user.id, account_id=account_id, title="Inny Czynsz", amount=Decimal("-1000.00"), frequency=Frequency.MONTHLY, day_of_month=1, start_date=date(2024, 1, 1), next_run_date=date(2024, 6, 1))
    ...
    assert len(data) == 2 # Powinny być widoczne tylko transakcje zalogowanego użytkownika
...
def test_api_update_recurring_transaction(client, app, setup_for_recurring):
    ...
    # SETUP - dodajemy definicję transakcji cyklicznej do edycji
    with app.app_context():
    ...
    # ACTION - symulujemy wysłanie żądania PUT z nowymi danymi
    updated_data = {
        'title': 'Nowy Czynsz',
        'amount': "-1200.00",
        'day_of_month': 15,
...
def test_api_delete_recurring_transaction(client, app, setup_for_recurring):
    ...
    # SETUP - dodajemy definicję transakcji cyklicznej do usunięcia
    with app.app_context():
        rec_tx = RecurringTransaction(
            user_id=user_id,
            account_id=account_id,
    ...
    # ACTION - symulujemy wysłanie żądania DELETE
    ...
    with app.app_context():
        ...
        assert deleted_rec_tx is None # Sprawdzamy, czy transakcja została usunięta z bazy